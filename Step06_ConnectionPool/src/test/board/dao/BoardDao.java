...
public class BoardDao {
   ...
   public BoardDto getData(int num){
      ...
      try{
         conn=new DbcpBean().getConn();
         String sql="SELECT writer,title,content,TO_CHAR(regdate,'YY/MM/DD HH24:Mi') as regdate"
               + " FROM board_guest"
               + " WHERE num=?";
         pstmt=conn.prepareStatement(sql);
         pstmt.setInt(1, num);
   ...
   public List<BoardDto> getList(){
      ...
      try{
         conn=new DbcpBean().getConn();
         String sql="SELECT num,writer,title,TO_CHAR(regdate,'YY/MM/DD HH24:Mi') as regdate"
               + " FROM board_guest"
               + " ORDER BY num DESC";
         pstmt=conn.prepareStatement(sql);
         //sql 문 수행하고 결과셋 받아오기 
